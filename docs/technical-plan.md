...
当前最高价值验证改为机制隔离：在相同双讲输入上同时导出 AEC3 的 16 kHz 线性滤波输出和
完整输出，并以同采样率试听。若线性输出保留近端音节而完整输出吞字，问题主要位于 residual
echo suppressor；若两者都吞字，则优先检查线性滤波、双讲检测或参考流对齐。导出只使用
上游已有的 `GetLinearAecOutput` 诊断接口，不启用独立 NS，也不修改 AEC3 算法源码。
