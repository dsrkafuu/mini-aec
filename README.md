...
- The active `production-driver-lifecycle` change adds the production release manifest, package preflight, compatibility and lifecycle contracts without changing the audio path. Production signing, installer implementation and real Windows lifecycle acceptance remain external-gated; the development validation package is not a production release.
- OpenSpec uses the `spec-driven` schema. The accepted M1, M2, M3, normal-user-access and M4 long-run-stability changes are archived and their capabilities are synced to the main specs. `production-driver-lifecycle` is the active M5 change.
...
The production package and lifecycle contract is documented in [docs/production-driver-lifecycle.md](docs/production-driver-lifecycle.md).
...
crates/mini-aec-release/   production manifest, package preflight and lifecycle model
