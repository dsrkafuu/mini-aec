...
/// Number of samples in WebRTC's exported 10 ms mono linear AEC frame at 16 kHz.
pub const LINEAR_AEC_OUTPUT_SAMPLES: usize = 160;
...
impl Processor {
    ...
    /// Returns the most recently produced 10 ms mono linear AEC output at 16 kHz.
    ///
    /// Returns [`None`] when the processor was not created with
    /// `filter.export_linear_aec_output`, full AEC is disabled, or no capture
    /// frame has produced an output yet.
    pub fn get_linear_aec_output(&self) -> Option<[f32; LINEAR_AEC_OUTPUT_SAMPLES]> {
        let mut output = [0.0; LINEAR_AEC_OUTPUT_SAMPLES];
        let available = unsafe { ffi::get_linear_aec_output(*self.inner, output.as_mut_ptr()) };
        available.then_some(output)
    }
...
mod tests {
    ...
    fn test_full_aec_with_linear_aec_output_correct() {
        ...
        context.processor.process_render_frame(&mut render).unwrap();
        context.processor.process_capture_frame(&mut capture).unwrap();
        assert!(context.processor.get_linear_aec_output().is_some());
    }
