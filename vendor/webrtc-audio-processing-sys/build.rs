...
mod webrtc {
    ...
    pub(super) fn build_if_necessary() -> Result<()> {
        ...
        // Copy the sources under OUT_DIR so patches and generated files never touch the vendor
        // tree. Using fs_extra keeps bundled builds independent of Unix `cp` on Windows.
        let mut copy_options = CopyOptions::new();
        copy_options.content_only = true;
        copy_options.overwrite = true;
        fs_extra::dir::copy(bundled_source_path, &webrtc_source_dir, &copy_options)
            .context("copying bundled WebRTC sources")?;
        ...
        if cfg!(target_env = "msvc") {
            // The bundled WebRTC sources use designated initializers that MSVC
            // only accepts in C++20 mode.
...
fn main() -> Result<()> {
    ...
    if cfg!(target_env = "msvc") {
        ...
        cc_build.std("c++17").flag("-Wno-unused-parameter");
    }
    ...
    if cfg!(feature = "bundled") {
        if cfg!(target_env = "msvc") {
            println!("cargo:rustc-link-lib=static:-bundle,+verbatim=lib{LIB_NAME}.a");
        } else {
