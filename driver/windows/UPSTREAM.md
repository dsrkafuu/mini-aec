...
11. `driver/windows/scripts/verify-upstream.ps1`: continues to require the exact imported file set and byte equality for every unmodified file while allowing only the seven vendored paths listed above to differ from the pinned commit.
...
13. `driver/windows/vendor/sysvad/TabletAudioSample/micintopo.cpp` and `micintoptable.h`: route the MiniAEC topology peak-meter property to a project-specific handler backed by the PCM16 samples delivered to WaveRT, returning zero for silent output and clearing the meter when transport audio resets; build verification is covered by [production-driver-package task 4.5](../../openspec/changes/production-driver-package/tasks.md#4-reproducibility-and-repository-verification) and was checked with the runtime-access policy verifier, isolated x64 Debug/Release MSBuild builds, and unsigned Windows 11 x64 Inf2Cat generation (no OpenSpec task was added for this isolated defect fix).
