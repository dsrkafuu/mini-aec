...
const GUID MiniAecTransportClassGuid = {
    ...
    0xf2d5,
    0x4f47,
    {0x9d, 0x73, 0xb2, 0x44, 0x36, 0x70, 0x6f, 0x87}};

NTSTATUS CompleteIrp(_In_ PIRP Irp, _In_ NTSTATUS Status,
...
NTSTATUS ForwardIrp(_In_ PDEVICE_OBJECT DeviceObject, _In_ PIRP Irp,
...
BOOLEAN SessionIdEquals(_In_reads_(16) const UCHAR *Left,
...
NTSTATUS ValidateCommonHeader(_In_ ULONG Magic, _In_ USHORT ProtocolVersion,
...
NTSTATUS HandleOpenSession(_In_ PFILE_OBJECT FileObject,
                           _In_reads_bytes_(InputLength)
                               const VOID *InputBuffer,
                           _In_ ULONG InputLength) {
  if (InputBuffer == nullptr ||
      InputLength != sizeof(MINIAEC_OPEN_SESSION_REQUEST)) {
    return STATUS_INVALID_BUFFER_SIZE;
  }

  const auto request =
      static_cast<const MINIAEC_OPEN_SESSION_REQUEST *>(InputBuffer);
  NTSTATUS status = ValidateCommonHeader(
      request->Magic, request->ProtocolVersion, request->HeaderSize,
      sizeof(MINIAEC_OPEN_SESSION_REQUEST), request->TotalSize,
  ...
  if (!IsOwnerLocked(FileObject)) {
    ...
    ResetAudioLocked();
    RtlCopyMemory(g_State.SessionId, request->SessionId,
                  sizeof(g_State.SessionId));
    g_State.HasLastAcceptedSequence = FALSE;
    g_State.LastAcceptedSequence = 0;
    g_State.NextSequence = 0;
...
NTSTATUS HandleWriteFrame(_In_ PFILE_OBJECT FileObject,
                          _In_reads_bytes_(InputLength) const VOID *InputBuffer,
                          _In_ ULONG InputLength) {
  if (InputBuffer == nullptr ||
      InputLength != sizeof(MINIAEC_WRITE_FRAME_REQUEST)) {
    IncrementRejectedWrite();
    return STATUS_INVALID_BUFFER_SIZE;
  }

  const auto request =
      static_cast<const MINIAEC_WRITE_FRAME_REQUEST *>(InputBuffer);
  NTSTATUS status = ValidateCommonHeader(
      request->Magic, request->ProtocolVersion, request->HeaderSize,
      FIELD_OFFSET(MINIAEC_WRITE_FRAME_REQUEST, Pcm), request->TotalSize,
      sizeof(MINIAEC_WRITE_FRAME_REQUEST));
  if (!NT_SUCCESS(status) || request->PayloadSize != MINIAEC_FRAME_BYTES ||
      request->Reserved != 0) {
    IncrementRejectedWrite();
  ...
  if (!IsOwnerLocked(FileObject)) {
    ...
    status = STATUS_INVALID_DEVICE_STATE;
             ...
             (g_State.HasLastAcceptedSequence &&
              g_State.LastAcceptedSequence == MAXULONGLONG)) {
...
NTSTATUS HandleGetDiagnostics(_In_ PFILE_OBJECT FileObject,
  ...
  diagnostics->SessionState =
      g_State.SessionActive ? MiniAecSessionOpen : MiniAecSessionClosed;
  diagnostics->CurrentDepth = g_State.RingCount;
  diagnostics->HighWaterMark = g_State.HighWaterMark;
  RtlCopyMemory(diagnostics->SessionId, g_State.SessionId,
                sizeof(diagnostics->SessionId));
...
NTSTATUS HandleCloseSession(_In_ PFILE_OBJECT FileObject,
                            _In_reads_bytes_(InputLength)
                                const VOID *InputBuffer,
                            _In_ ULONG InputLength) {
  if (InputBuffer == nullptr ||
      InputLength != sizeof(MINIAEC_CLOSE_SESSION_REQUEST)) {
    return STATUS_INVALID_BUFFER_SIZE;
  }

  const auto request =
      static_cast<const MINIAEC_CLOSE_SESSION_REQUEST *>(InputBuffer);
  NTSTATUS status = ValidateCommonHeader(
      request->Magic, request->ProtocolVersion, request->HeaderSize,
      sizeof(MINIAEC_CLOSE_SESSION_REQUEST), request->TotalSize,
...
_Dispatch_type_(IRP_MJ_CREATE) NTSTATUS
    MiniAecDispatchCreate(_In_ PDEVICE_OBJECT DeviceObject, _In_ PIRP Irp) {
...
_Dispatch_type_(IRP_MJ_CLEANUP) NTSTATUS
    MiniAecDispatchCleanup(_In_ PDEVICE_OBJECT DeviceObject, _In_ PIRP Irp) {
  if (DeviceObject != g_State.ControlDevice) {
...
_Dispatch_type_(IRP_MJ_CLOSE) NTSTATUS
    MiniAecDispatchClose(_In_ PDEVICE_OBJECT DeviceObject, _In_ PIRP Irp) {
  if (DeviceObject != g_State.ControlDevice) {
...
_Dispatch_type_(IRP_MJ_DEVICE_CONTROL) NTSTATUS
    MiniAecDispatchDeviceControl(_In_ PDEVICE_OBJECT DeviceObject,
                                 _In_ PIRP Irp) {
  if (DeviceObject != g_State.ControlDevice) {
  ...
  case IOCTL_MINIAEC_GET_DIAGNOSTICS:
    status = HandleGetDiagnostics(stack->FileObject, systemBuffer, outputLength,
                                  &information);
    break;
...
_IRQL_requires_max_(PASSIVE_LEVEL) NTSTATUS
  ...
  NTSTATUS status =
      IoCreateDeviceSecure(DriverObject, 0, &deviceName, FILE_DEVICE_UNKNOWN,
                           FILE_DEVICE_SECURE_OPEN, FALSE, &securityDescriptor,
                           &MiniAecTransportClassGuid, &g_State.ControlDevice);
  if (!NT_SUCCESS(status)) {
  ...
  while (destinationOffset < ByteCount) {
    if (g_State.CaptureOffset == MINIAEC_FRAME_BYTES) {
      if (g_State.RingCount != 0) {
        RtlCopyMemory(g_State.CaptureFrame, g_State.Ring[g_State.ReadIndex].Pcm,
                      MINIAEC_FRAME_BYTES);
        g_State.ReadIndex = (g_State.ReadIndex + 1) % MINIAEC_RING_CAPACITY;
        --g_State.RingCount;
      } else {
    ...
    RtlCopyMemory(Buffer + destinationOffset,
                  g_State.CaptureFrame + g_State.CaptureOffset, copyLength);
