...
- [x] 4.6 Remove any endpoint property that prevents `MiniAEC Microphone` from being selected as a default input device, update package guards to reject such a property, rebuild the x64 Debug validation package, and statically verify that it still declares exactly one public capture endpoint and no public render or producer-only endpoint.

## 5. Approved Driver Lifecycle Validation

- [x] 5.1 Present the exact test-mode, certificate, install, device-restart and uninstall commands plus rollback plan, including the separately approved restoration of saved default input roles if Windows does not restore them automatically, and obtain explicit user approval before each system-changing validation action.
- [x] 5.2 Save a pre-install inventory of physical audio endpoints, default devices, relevant driver packages, services, certificates and test-signing state.
- [x] 5.3 Install the approved test-signed validation package and verify that Windows adds exactly one public capture endpoint named `MiniAEC Microphone`, adds no public render or producer-only audio endpoint, allows the user to select it as a default input device in Windows system sound settings, records whether Windows automatically changes each default input role, and leaves unrelated physical endpoints unchanged.
- [x] 5.4 Record at least five minutes from `MiniAEC Microphone` with Windows Recorder and verify expected duration, ordered markers and diagnostics with no unexplained gap, stale segment, underrun, overflow or rejected write.
- [x] 5.5 While one recording remains open, stop the sender, verify continuing silence without stale replay, restart the sender and verify capture resumes with a new session and no old-session PCM.
- [x] 5.6 Execute the approved driver restart, reopen Windows Recorder after the endpoint returns, and verify a fresh sender session can be captured without reinstalling the package or exposing pre-restart PCM.
- [x] 5.7 Execute the approved uninstall and rollback procedure, verify the endpoint, control interface, service, staged validation package and targeted test certificate are gone, and compare unrelated physical endpoints, default input roles and boot state with the saved baseline; if Windows does not automatically restore a saved default input role, report the exact difference, obtain explicit approval, restore it, and only then declare rollback complete.
...
- [x] 6.3 Update the validation documentation with system-default selectability, accepted Windows-originated default-role changes and rollback behavior; record clean-checkout commands, environment, evidence paths, actual default-role changes, pass/fail and rollback results, and unresolved limitations; run the required workspace checks and confirm that no generated driver package, certificate private key or private recording is committed.
