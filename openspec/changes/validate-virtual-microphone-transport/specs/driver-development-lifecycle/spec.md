...
The validation workflow SHALL treat test-mode changes, certificate installation, driver installation, device restart and driver removal as system-changing actions that require explicit user approval before execution.
...
- **THEN** it reports prerequisites and the commands that would run without modifying boot configuration, certificate stores, drivers or device state
...
### Requirement: Test-signed installation isolation
The validation package SHALL use development-only identities and test signing, SHALL identify itself as non-production, and SHALL install without changing the default input device or altering unrelated physical audio endpoints.

#### Scenario: Validation driver is installed
- **WHEN** the approved test-signed package is installed on the development machine
- **THEN** `MiniAEC Microphone` appears without replacing the current default input device
...
The validation workflow SHALL remove the validation device, driver package, service registrations and validation-only control interfaces, and SHALL provide checks that compare the resulting audio device state with the saved pre-install baseline.
...
- **WHEN** any endpoint, service, package or validation certificate targeted for removal remains
- **THEN** the workflow reports the exact remaining identity and does not claim that rollback succeeded
